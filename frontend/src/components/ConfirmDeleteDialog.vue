...
const resolvedConfirm = computed(() => props.confirmLabel || t.value.delete)
...
<template>
  <div v-if="modelValue" class="modal-backdrop" @pointerdown.self="closeOnBackdrop && requestClose()">
    <section class="confirm-dialog" role="dialog" aria-modal="true" aria-labelledby="confirm-delete-title" :aria-describedby="description ? 'confirm-delete-desc' : undefined">
      <h2 id="confirm-delete-title">{{ title }}</h2>
      <p v-if="description" id="confirm-delete-desc">{{ description }}</p>
      <slot />
      <div class="confirm-dialog__actions">
        <button type="button" class="secondary-button" :disabled="busy" @click="requestClose">{{ resolvedCancel }}</button>
        <button type="button" class="danger-button danger-button--solid" :disabled="busy" @click="requestConfirm">
