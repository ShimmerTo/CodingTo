...
function compileRender(template, bindingMetadata) {
  const { code } = compile(template, {
    mode: 'function', prefixIdentifiers: true, hoistStatic: false, cacheHandlers: false, bindingMetadata,
  })
...
const nodeOps = {
  ...
  setElementText: (el, t) => { el.children = [makeText(t)] },
  ...
  nextSibling: (n) => { const p = n.parent; if (!p) return null; const i = p.children.indexOf(n); return p.children[i + 1] || null },
  querySelector: () => null, setScopeId: () => {},
  insertStaticContent: () => { const el = makeEl('static'); return [el, el] },
}
...
// Mirror ChatMessageItem tool message structure
    ...
    <details class="tool-call" :open="!!editDiff">
      <summary>
        ...
        <span class="tool-call__state">
          <CheckCircle2 v-if="done"/><LoaderCircle v-else/>
        </span>
        <span class="tool-call__name">{{ toolName }}</span>
      </summary>
      <div v-if="editDiff" class="tool-edit-diff"></div>
      <div v-else class="tool-call__details">{{ detail }}</div>
    </details>
    <div class="message-content">{{ now }}</div>
  </article>`,
  { icon: 'setup', editDiff: 'setup', done: 'setup', toolName: 'props', detail: 'props', now: 'props' }
)
const Child = defineComponent({
  props: { message: Object, now: Number },
  setup(props) {
    return {
      icon: computed(() => toolIcon(props.message)),
      editDiff: computed(() => props.message.editDiff || null),
      done: computed(() => props.message.done === true),
      toolName: computed(() => props.message.toolName || ''),
      detail: computed(() => props.message.detail || ''),
    }
  },
  render: childRender,
})
...
const Parent = defineComponent({
  setup() {
    ...
    const messages = ref([
      { id: '1', toolName: 'read', done: false, editDiff: null, live: true },
      { id: '2', toolName: 'search', done: true, editDiff: null, live: false },
      { id: '3', toolName: 'calc', done: true, editDiff: { x: 1 }, live: false },
    ])
    const needsRuntimeUpdate = (m) => m.live === true
    Object.assign(controls, { runtimeNow, messages })
    return { messages, needsRuntimeUpdate, runtimeNow }
  },
  render: parentRender,
})
...
app.config.errorHandler = (err) => { errored = true; if (!firstErr) firstErr = err && err.message; console.error('CAUGHT:', err && err.message) }
app.mount(host)
...
const timer = setInterval(() => {
  ...
  ticks++
  if (ticks === 10) {
    // simulate tool finishing: id 1 done, editDiff added
    controls.messages.value[0].done = true
    controls.messages.value[0].editDiff = { changes: 5 }
    controls.messages.value[0].live = false
    controls.runtimeNow.value = Date.now()
  }
  if (ticks >= 30) {
    clearInterval(timer)
    writeFileSync('_repro_result.txt', 'errored=' + errored + ' firstErr=' + firstErr + ' ticks=' + ticks + '\n')
